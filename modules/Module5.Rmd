...
# how would you define an xgboost model?
#xgboost<-

```

Note, XGBoost is robust against highly skewed and/or correlated data, so the amount of preprocessing required is minimal. 
...
The goal is to use what we have learned and run xgboost and glm.

```{r}


# Example dataset: keep binary classification
data(iris)
...
xgb_recipe <- recipe(Species ~ ., data = iris_train) %>%
...
xgb_grid <- grid_latin_hypercube(
  ...
  mtry(range = c(1, ncol(iris_train) - 1)),
  size = 20
)
...
final_fit <- fit(final_xgb, data = iris_train)

# Evaluate on test set
iris_test %>%
  predict(final_fit, ., type = "prob") %>%
  bind_cols(iris_test) %>%
  roc_auc(truth = Species, .pred_versicolor)  # AUROC

iris_test %>%
  predict(final_fit, ., type = "class") %>%
  bind_cols(iris_test) %>%
  accuracy(truth = Species, estimate = .pred_class)  # Accuracy
